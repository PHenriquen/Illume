...
function appRegistryPath() {
    return path.join(app.getPath('userData'), 'approved-apps.json');
}

function routineRegistryPath() {
    return path.join(app.getPath('userData'), 'routines.json');
}

function loadApprovedApps() {
    try {
        approvedApps = JSON.parse(fs.readFileSync(appRegistryPath(), 'utf8'))
            .filter(item => item && (item.kind === 'uwp' || fs.existsSync(item.path)));
    }
    catch {
        approvedApps = [];
    }
...
function saveApprovedApps() {
    fs.writeFileSync(appRegistryPath(), JSON.stringify(approvedApps, null, 2), 'utf8');
}

function loadRoutines() {
    try {
        routines = JSON.parse(fs.readFileSync(routineRegistryPath(), 'utf8'))
            .filter(item => item?.name && Array.isArray(item.apps));
    }
    catch {
        routines = [];
    }
...
function saveRoutines() {
    fs.writeFileSync(routineRegistryPath(), JSON.stringify(routines, null, 2), 'utf8');
}

function cleanAppName(filePath) {
...
function collectShortcuts(directory, output, depth = 0) {
    ...
    try {
        ...
        // Algumas pastas do Windows podem estar protegidas ou desaparecer durante a varredura.
    }
}
...
function detectInstalledApps() {
    ...
    if (process.platform === 'win32') {
        try {
            ...
            // Os atalhos clássicos continuam disponíveis se a consulta da Store falhar.
        }
    }
...
function startBackend() {
    ...
    backend = spawn(python.command, [...python.args, '-m', 'backend.launcher'], {
        cwd: coreDirectory(),
        windowsHide: true,
        env: { ...process.env, NOA_NO_BROWSER: '1' },
        stdio: 'ignore'
    });
}
        ...
        wakeListener.stdout.on('data', chunk => {
            ...
            for (const line of lines) {
                try {
                    ...
                    // O PowerShell pode escrever linhas auxiliares que não fazem parte do protocolo JSON.
                }
            }
        });

        wakeListener.on('exit', () => {
            wakeListener = null;
            wakeListenerReady = false;
            sendToWindows('trace:wake-status', { ready: false, reason: 'listener_stopped' });
        });
    }
    catch {
        wakeListener = null;
...
function cleanupLegacyVoiceProcesses() {
    ...
    // Instalações antigas usavam TRACE-AI no caminho/comando do Whisper.
    ...
    try {
        ...
        // Limpeza opcional: a aplicação pode continuar mesmo se o processo antigo não existir.
    }
}
...
function showOverlay() {
    ...
    overlayTransition++;
    overlay.setShape([]);
    overlay.setBounds(overlayBounds(), false);
    overlayMode('chat-enter');
    overlay.setOpacity(0);
    overlay.show();
    compactExpanded = true;
    surfaceState = 'chat';
    animateOverlayOpacity(0, 1, 220, () => {
        overlayMode('chat');
        overlay?.focus();
    });
}
...
function hideOverlay() {
    ...
    animateOverlayOpacity(overlay.getOpacity(), 0, 180, () => {
        if (!overlay || overlay.isDestroyed() || !['hidden', 'dashboard'].includes(surfaceState))
            return;
        overlay.setShape([]);
        overlay.setBounds(parkedOverlayBounds(), false);
        overlayMode('hidden');
    });
}
...
function openCommandPalette() {
    showOverlay();
    setTimeout(() => overlay?.webContents.send('trace:global-command'), 120);
}
...
async function capturePrimaryScreen() {
    try {
        ...
        const sources = await desktopCapturer.getSources({
            types: ['screen'],
            thumbnailSize: { width: Math.min(1440, size.width), height: Math.min(900, size.height) },
...
async function generateDiagnostic() {
    ...
    const report = {
        ...
        runtime: {
            ...
            wakeListenerReady,
            backendRunning: Boolean(backend && !backend.killed),
            rendererMemoryMB: Math.round((memory.workingSetSize || 0) / 1024),
            state: runtimeState.state,
            micStatus: runtimeState.micStatus
        },
        configuration: {
    ...
    const result = await dialog.showSaveDialog(dashboard, {
        title: `Salvar diagnóstico da ${PRODUCT_NAME}`,
        defaultPath: `${PRODUCT_NAME}-diagnostico-${new Date().toISOString().slice(0, 10)}.json`,
...
async function launchApprovedApp(item) {
    if (item.kind === 'uwp') {
        try {
            spawn('explorer.exe', [`shell:AppsFolder\\${item.path}`], {
                windowsHide: true,
                detached: true,
                stdio: 'ignore'
            }).unref();
...
// Os canais `trace:*` continuam temporariamente para não quebrar a bridge já instalada.
ipcMain.handle('trace:expand-overlay', () => { showOrb(); return true; });
ipcMain.handle('trace:show-orb', () => activateOrb());
ipcMain.handle('trace:toggle-compact', () => {
    compactExpanded ? showOrb(true) : showOverlay();
...
ipcMain.handle('trace:calibrate-claps', () => {
    if (overlay && !overlay.isDestroyed())
        overlay.webContents.send('trace:clap-calibration', 'start');
    return true;
});
ipcMain.handle('trace:reset-claps', () => {
    if (overlay && !overlay.isDestroyed())
        overlay.webContents.send('trace:clap-calibration', 'reset');
    return true;
});
ipcMain.handle('trace:start-listening', () => {
    if (overlay && !overlay.isDestroyed())
        overlay.webContents.send('trace:start-listening');
    return true;
});
ipcMain.handle('trace:set-ambient', (_event, enabled) => {
    if (overlay && !overlay.isDestroyed())
        overlay.webContents.send('trace:ambient-control', Boolean(enabled));
    return true;
});
...
ipcMain.handle('trace:select-files', async () => {
    const result = await dialog.showOpenDialog(dashboard, {
        title: `Anexar à ${PRODUCT_NAME}`,
        properties: ['openFile', 'multiSelections'],
        filters: [
            { name: 'Arquivos compatíveis', extensions: ['pdf', 'docx', 'txt', 'md', 'csv', 'json', 'js', 'ts', 'py', 'html', 'css', 'png', 'jpg', 'jpeg', 'webp'] },
...
ipcMain.handle('trace:save-document', async (_event, data) => {
    ...
    const result = await dialog.showSaveDialog(dashboard, {
        title: `Salvar resposta da ${PRODUCT_NAME}`,
        defaultPath: `resposta-noa.${format}`,
        filters
    });
    if (result.canceled || !result.filePath)
        return false;
    ...
    if (format === 'docx' && data?.bytes) {
        ...
        const html = `<style>body{font:15px/1.6 Segoe UI,sans-serif;color:#17202a;padding:40px;white-space:pre-wrap}h1{font-size:20px}</style><h1>Documento revisado pela Noa</h1><div>${escaped}</div>`;
        await printWindow.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(html)}`);
        fs.writeFileSync(
            result.filePath,
            await printWindow.webContents.printToPDF({ printBackground: true, pageSize: 'A4' })
        );
        printWindow.destroy();
    }
    else {
...
ipcMain.handle('trace:select-app', async () => {
    const result = await dialog.showOpenDialog(dashboard, {
        title: 'Autorizar aplicativo',
        properties: ['openFile'],
        filters: [{ name: 'Aplicativos Windows', extensions: ['exe', 'lnk'] }]
    });
    if (result.canceled)
        return null;
    ...
    if (!approvedApps.some(entry => entry.path === appPath)) {
...
ipcMain.handle('trace:detect-apps', () => {
    const scanned = detectInstalledApps();
    const paths = new Set(scanned.map(item => item.path));
    return [...scanned, ...approvedApps.filter(item => !paths.has(item.path))]
        .map(item => ({ ...item, authorized: approvedApps.some(entry => entry.path === item.path) }));
});
...
ipcMain.handle('trace:set-app-authorized', (_event, item, authorized) => {
    ...
    const candidate = {
        name: String(item.name || cleanAppName(item.path)),
        path: String(item.path),
        ...(item.kind === 'uwp' ? { kind: 'uwp' } : {})
    };
...
ipcMain.handle('trace:save-routine', (_event, data) => {
    const name = String(data?.name || '').trim().slice(0, 40);
    const apps = Array.isArray(data?.apps)
        ? data.apps.filter(appName => approvedApps.some(item => item.name === appName)).slice(0, 10)
        : [];

    if (!name || !apps.length)
        return false;

    routines = routines.filter(item => item.name.toLocaleLowerCase('pt-BR') !== name.toLocaleLowerCase('pt-BR'));
    routines.push({ name, apps });
    saveRoutines();
    return true;
});

ipcMain.handle('trace:delete-routine', (_event, name) => {
    routines = routines.filter(item => item.name !== name);
    saveRoutines();
    return true;
});
...
ipcMain.handle('trace:mic-level', (_event, data) => {
    runtimeState = {
        ...runtimeState,
        micLevel: Number(data?.level || 0),
        ...(data?.status ? { micStatus: String(data.status) } : {})
    };
...
ipcMain.handle('trace:runtime-state', () => ({
    ...runtimeState,
    surface: surfaceState,
    wakeListenerReady,
    approvedApps: approvedApps.length,
...
ipcMain.handle('trace:set-startup', (_event, enabled) => {
...
app.whenReady().then(async () => {
    ...
    try {
        ...
        // A interface exibirá o estado offline.
    }
...
app.on('before-quit', () => {
    ...
    cleanupLegacyVoiceProcesses();
    if (wakeListener && !wakeListener.killed)
        wakeListener.kill();
