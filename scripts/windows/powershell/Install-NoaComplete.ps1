...
$LegacyDataDirectory = Join-Path $env:LOCALAPPDATA 'TRACE-AI'
...
Write-Host 'Noa — instalação completa e restaurável' -ForegroundColor White
Write-Host 'Aplicativo, IA local, voz, documentos, testes e instalador do Windows.'
Write-Host 'Os dados existentes do TRACE serão preservados durante esta fase de migração.' -ForegroundColor DarkGray
...
& npm run check
if ($LASTEXITCODE -ne 0) {
    throw 'A validação do projeto falhou.'
}

if (-not $SkipBuild) {
    Write-Step 'Compilando a interface da Noa'
    & npm run build
    if ($LASTEXITCODE -ne 0) {
        throw 'O build da interface falhou.'
    }
}
...
if (-not $SkipInstaller) {
    Write-Step 'Gerando o instalador da Noa para Windows'
...
if ($LASTEXITCODE -ne 0) {
    Write-Warning 'A Noa foi preparada, mas algum componente opcional ainda está ausente.'
}

Write-Host "`nNoa preparada com sucesso." -ForegroundColor Green
Write-Host "Dados locais preservados: $LegacyDataDirectory"
